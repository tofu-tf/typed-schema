...
object FindKey extends FindSymbolicKey {
  ...
  final implicit def findStringKey[Params <: HList, name <: String](implicit
...
trait FindSymbolicKey {
  final implicit def findSymbolicKey[Params <: HList, name <: String](implicit
      selectKey: Find.Aux[Key, Params, Symbol with Tagged[name]],
      keyValue: Witness.Aux[name]
  ): Aux[Params, name] = new FindKey[Params] {
    type Out = name
    def apply(): name = keyValue.value
