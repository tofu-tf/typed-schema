...
object BinBody {
  implicit def binBodyServe[F[_]: FlatMap, name, In <: HList](
      implicit A: BinParseBody[F, Array[Byte]]
  ): Add[BinBody[name, Array[Byte]], F, In, name, Array[Byte]] =
    add[BinBody[name, Array[Byte]], F, In, Array[Byte], name](A.parse())

  implicit def binBodySwagger[name](
      implicit ev: SwaggerMapper[ReqBody[name, Array[Byte]]]
