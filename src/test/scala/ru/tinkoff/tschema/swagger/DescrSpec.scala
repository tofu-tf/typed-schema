...
class DescrSpec extends FlatSpec {
  "descr" should "set description for existing field" in assert(
    SwaggerTypeable
      ...
      Vector(("x", Some(xDescr), SwaggerPrimitive.string),
  ...
  "DescribeTypeable" should "set description for type" in assert(barTyp match { case ref: SwaggerRef => ref.descr.contains(barDescr) })
  it should "set description for fields" in assert(barTyp.deref.value match {
    case obj: SwaggerObject => obj.properties.forall {
